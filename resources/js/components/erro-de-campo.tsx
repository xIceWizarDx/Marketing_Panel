 ...
 */
export default function ErroDeCampo({
    mensagem,
    className,
    ...props
}: HTMLAttributes<HTMLParagraphElement> & { mensagem?: string }) {
    if (!mensagem) {
        return null;
    }

    return (
        <p {...props} role="alert" className={cn('text-[color:var(--destructive)] text-sm', className)}>
            {mensagem}
        </p>
    );
}
